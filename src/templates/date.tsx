...
const toSubDate = (type: string, subDate: string, date: string) => {
  if (type === 'year') {
    return monthNames[parseInt(subDate, 10) - 1]
  } else {
    const [year, month] = date.split('-')

    return `${monthNames[parseInt(month, 10) - 1]} ${subDate}, ${year}`
  }
}

const DateTemplate = ({ pageContext }: PageProps) => {
  const { postByDate: posts, siteTitle, dateType: type, total, date } = pageContext!
        ...
        <div className="blog-container">
          <div className="card">
            <div className="content page-content page-fontsize">
              {Object.keys(groupDate).map(subDate => (
                <>
                  <h3>{toSubDate(type!, subDate, date!)}</h3>
                  <ul>
